"""Stage-one export: Val Best, then explicitly authorized fixed-Best Test."""
...
ALLOWED_EXPERIMENTS = tuple(PROFILES)
...
def main():
    args = parse_args()
    manifest=validate_manifest(json.loads((REPO_ROOT/'experiment_manifests'/'active_cos_restart.json').read_text()))
    assert args.experiment==manifest['profile'] and config.seed==manifest['seed']
    if args.split == 'test':
        if os.environ.get('TEST_SPLIT_ALLOWED') != '1':
            raise RuntimeError('Test is not authorized in this process')
        if args.threshold != .5 or args.batch_size != 16 or args.expected_best_epoch is None:
            raise RuntimeError('Fixed threshold, batch and Val-selected Best required')
    if args.output.exists():
        raise RuntimeError('Refuse to overwrite an existing evaluation')
    ...
    assert checkpoint['seed']==manifest['seed'] and checkpoint['epochs']==80
    assert checkpoint['race_route_enabled']==manifest['race_route_enabled']
    assert checkpoint['race_binding_repair']==manifest['race_binding_repair']
    assert checkpoint['decoder_fusion_mode']==manifest['decoder_fusion_mode']
    assert checkpoint['stage1_match_fsdr_initialization']==manifest['stage1_match_fsdr_initialization']
    ...
    result = {
        ...
        "stage1_match_fsdr_initialization": manifest["stage1_match_fsdr_initialization"],
        "split": args.split,
