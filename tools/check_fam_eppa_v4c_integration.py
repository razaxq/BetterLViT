"""CUDA and real-data integration check for FAM-EPPA V4-C."""
...
def main():
    ...
    if config.experiment_architecture_version != "fam_eppa_v4c":
        raise RuntimeError("The active configuration is not FAM-EPPA V4-C")
    ...
    if config.loss_name != "dice_focal" or config.resume_path:
        raise RuntimeError("V4-C must start with Dice/Focal from scratch")
    ...
    for stage in ("up4", "up3"):
        ...
        if not module.use_semantic_flow_alignment:
            raise RuntimeError("{} must enable semantic flow".format(stage))
    for stage in ("up2", "up1"):
        module = getattr(model, stage).eppa
        if module.use_adaptive_frequency or module.use_semantic_flow_alignment:
    ...
    for stage in ("up4", "up3"):
        ...
        require_gradient(
            "{} flow predictor".format(stage),
            adaptive.flow_predictor[-1].weight,
        )
        require_gradient(
    ...
    print("FAM-EPPA V4-C real-data integration check passed.")
