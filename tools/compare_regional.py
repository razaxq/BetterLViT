"""Frozen RS1/RS2/RS3 Val gates; reports all conditions without Test access."""
import argparse
import hashlib
import json
...
def compare(a,b,regional_increment=False):
    ...
    profiles=('rs1_global_iou','rs2_local_iou','rs3_balanced_iou')
    assert a['experiment'] in profiles if regional_increment else a['experiment']=='r2_single_cosine'
    assert b['experiment'] in profiles
    ...
    if regional_increment:
        checks={k:checks[k] for k in ('iou_minimum','iou_ci_positive')}
