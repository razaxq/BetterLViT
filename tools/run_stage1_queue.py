"""Exactly eight serial authorized runs; stop on failure, never auto-change config."""
import argparse,fcntl,json,subprocess,sys,time
from pathlib import Path
from run_stage1 import write
...
def main():
    ...
    assert len(spec['runs'])==8 and spec['scope']=='stage1_whole_module_only'
    assert len({r['source_git_commit'] for r in spec['runs']})==8
    ...
    try:
        for r in spec['runs']:
            ...
            with (stage/(r['run_id']+'_runner.log')).open('w') as log:
                proc=subprocess.Popen([sys.executable,'-u','tools/run_stage1.py','--run',str(run)],cwd=repo,
