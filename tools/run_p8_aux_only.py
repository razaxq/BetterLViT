...
from p8_aux_protocol import validate_manifest as validate_recipe_manifest
...
def main():
    ...
    manifest_path = repo / 'experiment_manifests' / 'active_aux_only.json'
    ...
    assert subprocess.check_output(['git','rev-parse',manifest['experiment_tag']+'^{commit}'], cwd=repo, text=True).strip() == commit
    args.run.mkdir(parents=True, exist_ok=False)
    lock = (args.run.parent / 'recipe_training.lock').open('w')
    ...
    try:
        ...
        assert validation['checkpoint_git_commit']==commit and validation['samples']==1429
        assert validation['race_route_enabled'] is False and validation['race_binding_repair'] is True
        state.update(phase='test', test_split_accessed=True, test_selected_best_epoch=validation['checkpoint_best_epoch'])
        write(args.run/'runtime.json', state)
        test_env=dict(env,TEST_SPLIT_ALLOWED='1')
        with (args.run/'test.log').open('w') as log:
            rc=subprocess.run([python, 'tools/export_validation_metrics.py', '--experiment',manifest['profile'],
                '--checkpoint',str(bests[0]),'--output',str(args.run/'test.json'),'--batch-size','16','--threshold','0.5',
                '--split','test','--expected-best-epoch',str(validation['checkpoint_best_epoch'])],
                cwd=repo,env=test_env,stdout=log,stderr=subprocess.STDOUT).returncode
        state.update(test_rc=rc,test_ended_unix=time.time())
        if rc:raise RuntimeError('Test export failed: '+str(rc))
        result=json.loads((args.run/'test.json').read_text())
        assert result['samples']==2113 and result['checkpoint_git_commit']==commit
