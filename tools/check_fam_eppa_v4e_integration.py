"""CUDA and real-data integration check for FAM-EPPA V4-E."""
...
def main():
    ...
    if config.experiment_architecture_version != "fam_eppa_v4e":
        raise RuntimeError("The active configuration is not FAM-EPPA V4-E")
    ...
    if config.loss_name != "dice_focal" or config.resume_path:
        raise RuntimeError("V4-E must start with Dice/Focal from scratch")

    batch = load_real_batch()
    model = build_model().cuda().train()
    up4 = model.up4.eppa
    if not up4.use_adaptive_frequency or not up4.use_plam_calibration:
        raise RuntimeError("up4 must enable frequency and PLAM calibration")
    if up4.use_token_routing or up4.use_semantic_flow_alignment:
        raise RuntimeError("up4 must disable token routing and semantic flow")
    torch.nn.init.normal_(up4.plam_calibrator[-1].weight, std=1e-3)

    up3 = model.up3.eppa
    if not up3.use_adaptive_frequency or up3.use_plam_calibration:
        raise RuntimeError("up3 must retain the uncalibrated V4-B path")
    ...
    require_gradient("up4 calibrator input", up4.plam_calibrator[0].weight)
    require_gradient("up4 calibrator output", up4.plam_calibrator[-1].weight)
    ...
    print("FAM-EPPA V4-E real-data integration check passed.")
