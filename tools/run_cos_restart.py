...
def main():
    ...
    repo=Path(__file__).resolve().parents[1];m=validate_manifest(json.loads((repo/'experiment_manifests/active_cos_restart.json').read_text()))
    ...
    try:
        ...
        for split,count in [('validation',1429),('test',2113)]:
            ...
            command=[python,'tools/export_cos_restart_metrics.py','--experiment',m['profile'],'--checkpoint',str(best),
