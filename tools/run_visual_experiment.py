...
def main():
    ...
    manifest_path = repo / 'experiment_manifests' / 'active_visual.json'
    ...
    assert not manifest['test_split_allowed'] and not manifest['lora'] and manifest['boundary_loss'] == 0
    assert manifest['profile'] in ('p12_visual_prior', 'p12_visual_natural', 'c9_visual_random')
    ...
    lock = (args.run.parent / 'visual_training.lock').open('w')
    ...
    env.update(BETTERLVIT_EXPERIMENT=manifest['profile'], BETTERLVIT_SEED=str(manifest['seed']),
        ...
        BETTERLVIT_VISUAL_MODEL_ROOT=str(args.models), CUBLAS_WORKSPACE_CONFIG=':4096:8',
    ...
    state = {'source_git_commit':commit, 'repository':str(repo), 'run':str(args.run),
        'manifest':manifest, 'external_weights':json.loads((args.models/'manifest.json').read_text()),
