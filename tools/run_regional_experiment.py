"""Immutable RS1/RS2/RS3 runner: full training, scheduled telemetry, then Val export."""
import argparse
import fcntl
import json
import os
...
from regional_protocol import validate_regional_manifest
...
def main():
    ...
    manifest=json.loads((repo/'experiment_manifests/active_regional.json').read_text())
    validate_regional_manifest(manifest)
    ...
    assert profile['regional_mode']==manifest['regional_mode']
    assert profile['regional_weight']==manifest['regional_weight']
    args.run.mkdir(parents=True,exist_ok=False)
    lock=(args.run.parent/'regional_training.lock').open('w')
    ...
    env=dict(os.environ,BETTERLVIT_EXPERIMENT=manifest['profile'],BETTERLVIT_SEED=str(manifest['seed']),
        ...
        BETTERLVIT_REGIONAL_AUDIT_PATH=str(args.run/'diagnostics'),
