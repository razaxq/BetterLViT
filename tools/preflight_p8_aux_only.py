...
os.environ.setdefault('BETTERLVIT_EXPERIMENT', 'p8_r2_binding_aux')
import numpy as np
import torch
...
def main():
    ...
    assert model.race.route_enabled is False
    assert all(route.strength_logit.grad is None and all(p.grad is None for p in route.residual.parameters()) for route in model.race.routes)
    model.zero_grad(set_to_none=True)
    aux_only=dict(pred,final=pred['final'].detach())
    auxiliary_loss=objective(aux_only,b['label'])
    auxiliary_loss.backward()
    assert any(p.grad is not None and p.grad.abs().max()>0 for p in model.down1.parameters()), 'Auxiliary gradient did not reach CNN encoder'
    assert model.race.slot_head[0].weight.grad.abs().max()>0
    assert all(route.evidence[-1].weight.grad.abs().max()>0 for route in model.race.routes)
    assert all(route.strength_logit.grad is None and all(p.grad is None for p in route.residual.parameters()) for route in model.race.routes)
    ...
    print(json.dumps({'route_disabled': True, 'auxiliary_encoder_gradient_verified': True, 'status': 'ok', 'experiment': config.experiment_name,
