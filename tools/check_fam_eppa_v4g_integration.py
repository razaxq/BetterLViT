"""GPU and real-data integration check for FAM-EPPA V4-G."""
...
def main():
    ...
    if config.experiment_architecture_version != "fam_eppa_v4g":
        raise RuntimeError("The active configuration is not FAM-EPPA V4-G")
    ...
    if config.loss_name != "dice_focal" or config.resume_path:
        raise RuntimeError("V4-G must start with Dice/Focal from scratch")
    ...
    if not up4.use_semantic_reliability_residual:
        raise RuntimeError("up4 must enable mean-preserving reliability")
    if (
        up4.use_plam_calibration
        or up4.use_token_routing
        or up4.use_semantic_flow_alignment
        or up4.use_semantic_prototype_aggregation
    ):
        raise RuntimeError("up4 must disable V4-C through V4-F branches")
    up4.semantic_reliability_strength_raw.data.fill_(
        math.atanh(0.05 / up4.semantic_reliability_strength_max)
    )

    up3 = model.up3.eppa
    if (
        not up3.use_adaptive_frequency
        or up3.use_semantic_reliability_residual
    ):
    ...
    require_gradient(
        "up4 reliability input",
        up4.semantic_reliability_calibrator[0].weight,
    )
    require_gradient(
        "up4 reliability output",
        up4.semantic_reliability_calibrator[-1].weight,
    )
    require_gradient(
        "up4 reliability strength",
        up4.semantic_reliability_strength_raw,
    )
    ...
    print("FAM-EPPA V4-G real-data integration check passed.")
