...
class LViT(nn.Module):
    def __init__(self, config, n_channels=3, n_classes=1, img_size=224, vis=False, text_seq_len=10):
        ...
        self.tcsr_version = str(getattr(config, 'tcsr_version', 'v1'))
        ...
        if self.tcsr_enabled:
            ...
            if self.tcsr_version not in router_types:
                raise ValueError(
                    'Unsupported TCSR version: {}'.format(self.tcsr_version)
                )
            router_kwargs = {
                'skip_channels': (
                    in_channels,
            ...
            if self.tcsr_version == 'v2.2':
                router_kwargs['initial_gate_probability'] = getattr(
                    config,
                    'tcsr_initial_gate_probability',
                    0.25,
                )
            if self.tcsr_version in ('v2.3', 'v2.4', 'v2.5'):
                router_kwargs.update({
                    'initial_gate_probability': getattr(
            ...
            if self.tcsr_version == 'v2.5':
                router_kwargs.update({
                    'localization_weight': getattr(
                        config, 'tcsr_localization_weight', 0.02,
                    ),
                    'residual_leakage_weight': getattr(
                        config, 'tcsr_residual_leakage_weight', 0.5,
                    ),
                })
