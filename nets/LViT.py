...
class LViT(nn.Module):
    def __init__(self, config, n_channels=3, n_classes=1, img_size=224, vis=False, text_seq_len=10):
        ...
        # Down path: cross-attention with text at every scale (Q=patches, K/V=text pyramid).
        self.downVit = VisionTransformer(config, vis, img_size=224, channel_num=64, patch_size=16, embed_dim=64, text_seq_len=text_seq_len, use_cross_attn=True)
        self.downVit1 = VisionTransformer(config, vis, img_size=112, channel_num=128, patch_size=8, embed_dim=128, text_seq_len=text_seq_len, use_cross_attn=True)
        self.downVit2 = VisionTransformer(config, vis, img_size=56, channel_num=256, patch_size=4, embed_dim=256, text_seq_len=text_seq_len, use_cross_attn=True)
        self.downVit3 = VisionTransformer(config, vis, img_size=28, channel_num=512, patch_size=2, embed_dim=512, text_seq_len=text_seq_len, use_cross_attn=True)
        # Up path (reconstruct): no cross-attn, top-down ViT refinement is text-free.
        self.upVit = VisionTransformer(config, vis, img_size=224, channel_num=64, patch_size=16, embed_dim=64, text_seq_len=text_seq_len, use_cross_attn=False)
        self.upVit1 = VisionTransformer(config, vis, img_size=112, channel_num=128, patch_size=8, embed_dim=128, text_seq_len=text_seq_len, use_cross_attn=False)
        self.upVit2 = VisionTransformer(config, vis, img_size=56, channel_num=256, patch_size=4, embed_dim=256, text_seq_len=text_seq_len, use_cross_attn=False)
        self.upVit3 = VisionTransformer(config, vis, img_size=28, channel_num=512, patch_size=2, embed_dim=512, text_seq_len=text_seq_len, use_cross_attn=False)
    ...
    def forward(self, x, text, text_mask=None):
        ...
        x3 = self.down2(x2)
        y3 = self.downVit2(x3, y2, text3, text_mask=text_mask)
        x4 = self.down3(x3)
        y4 = self.downVit3(x4, y3, text4, text_mask=text_mask)
        x5 = self.down4(x4)
