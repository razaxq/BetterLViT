...
REPO_ROOT = Path(os.environ["RECIPE_EVAL_REPO"]).resolve()
...
ALLOWED_EXPERIMENTS = ('s1_r2_pixel_aux', 's2_r2_visual_aux')
...
def main():
    ...
    if args.batch_size != 16 or args.output.exists():
        raise RuntimeError("Registered batch16 and unused output path required")
    authorization_path = Path(os.environ["VISUAL_AUX_TEST_AUTHORIZATION"])
    authorization = json.loads(authorization_path.read_text())
    authorization_sha = __import__("hashlib").sha256(authorization_path.read_bytes()).hexdigest()
    if authorization_sha != os.environ["VISUAL_AUX_TEST_AUTHORIZATION_SHA256"]:
        raise RuntimeError("Authorization content hash mismatch")
    if not authorization["explicit_user_requested_test"] or authorization["threshold"] != 0.5:
        raise RuntimeError("Explicit frozen-checkpoint Test request required")
    arm = next(x for x in authorization["arms"] if x["profile"] == args.experiment)
    if Path(arm["repository"]).resolve() != REPO_ROOT or Path(arm["checkpoint"]).resolve() != args.checkpoint.resolve():
        raise RuntimeError("Unregistered repository or checkpoint")
    if arm["source_git_commit"] != os.environ["BETTERLVIT_GIT_COMMIT"]:
        raise RuntimeError("Registered training source mismatch")
    if subprocess.check_output(["git", "-C", str(REPO_ROOT), "status", "--porcelain", "--untracked-files=no"], text=True).strip():
        raise RuntimeError("Training source worktree changed")
    ...
    expected_seed = int(os.environ["BETTERLVIT_SEED"])
    expected_best = int(os.environ["RECIPE_TEST_BEST_EPOCH"])
    if checkpoint.get("seed") != expected_seed or checkpoint.get("epochs") != 80 or checkpoint.get("best_epoch") != expected_best:
        raise RuntimeError("Frozen seed/epochs/Best mismatch")
    if checkpoint.get("selection_metric") != "iou":
        raise RuntimeError("Checkpoint must be selected by validation IoU")
    if getattr(config, "training_recipe_enabled", False):
        from training_recipe import recipe_metadata
        if checkpoint.get("training_recipe") != recipe_metadata(config):
            raise RuntimeError("Training recipe metadata mismatch")
    if any(bool(getattr(config, k, False)) for k in ("visual_prior_enabled", "bcdh_enabled", "cdrr_enabled", "text_use_lora")):
        raise RuntimeError("Unregistered model module enabled")
    if not config.race_enabled or not config.race_pe_enabled or config.race_pe_route_enabled:
        raise RuntimeError("Expected frozen visual-only auxiliary architecture")
    if config.visual_aux_mode != arm["visual_aux_mode"] or checkpoint.get("visual_aux_mode") != arm["visual_aux_mode"]:
        raise RuntimeError("Visual auxiliary mode mismatch")
    if expected_seed != arm["seed"] or expected_best != arm["best_epoch"]:
        raise RuntimeError("Authorization seed/Best mismatch")
    analysis_commit = git_commit()
    checkpoint_commit = checkpoint.get("source_git_commit")
    if checkpoint_commit != os.environ["BETTERLVIT_GIT_COMMIT"]:
        raise RuntimeError("Expected training source mismatch")
    ...
    result = {
        ...
        "evaluation_source_git_commit": os.environ["RECIPE_TEST_ANALYSIS_SHA"],
        "authorization_sha256": authorization_sha,
        "validation_screen_passed": False,
        "test_purpose": "User-requested single-seed comparison after failed validation screen",
        "visual_aux_mode": config.visual_aux_mode,
        "training_recipe": checkpoint.get("training_recipe"),
        "checkpoint_sha256": __import__("hashlib").file_digest(checkpoint_path.open("rb"), "sha256").hexdigest(),
