...
def validate_text_identity(dataset,bindings,tokenizer):
    expected_images=[n.replace('mask_','') for n in dataset.mask_list]
    assert dataset.images_list==expected_images and set(bindings)==set(expected_images)
    # Check every original text against the IDs actually stored by the frozen loader.
    ordered=[bindings[n]['text'] for n in expected_images]
    encoded=tokenizer(ordered,max_length=32,padding='max_length',truncation=True,return_tensors='pt')
    assert torch.equal(dataset.input_ids,encoded['input_ids'])
    assert torch.equal(dataset.attention_masks,encoded['attention_mask'])
    return len(ordered)


@torch.inference_mode()
def batch_diagnostic(model,tokenizer,policy,bindings,frequency,batch,names,verify_native=False):
    # Workbook keys and policy identities stay at mask level; metric names stay at image level.
    cases=[bindings[n] for n in names]
    texts={n:case['text'] for n,case in zip(names,cases)}
    original_encoded=tokenizer([texts[n] for n in names],max_length=32,padding='max_length',truncation=True,return_tensors='pt')
    assert torch.equal(batch['input_ids'],original_encoded['input_ids'])
    assert torch.equal(batch['attention_mask'],original_encoded['attention_mask'])
    ...
    rows=[]; transformed=[policy.variants(case['mask_name'],case['text']) for case in cases]
    for i,name in enumerate(names):
        rows.append(dict(name=name,mask_name=cases[i]['mask_name'],source_text=texts[name],variant_texts=transformed[i],
...
def main(args,runtime):
    ...
    assert digest(HERE/'text_policy.py')==MANIFEST['registered_text_policy_sha256']
    ...
    train_bindings=build_bindings(train.mask_list,texts)
    train_identity_count=validate_text_identity(train,train_bindings,tokenizer)
    ...
    for batch,names in preflight_loader:
        ...
        preflight.extend(batch_diagnostic(model,tokenizer,policy,train_bindings,frequency,batch,names,verify_native=True))
    ...
    write_json(args.output/'preflight.json',dict(status='ok',train_names=[r['name'] for r in preflight],
        ...
        train_text_identity_verified_samples=train_identity_count,policy_identity='mask_filename',metric_identity='image_filename',
    ...
    val_bindings=build_bindings(dataset.mask_list,texts)
    assert set(refs)==set(val_bindings)
    val_identity_count=validate_text_identity(dataset,val_bindings,tokenizer)
    max_difference=0.;rows=[]
    for batch,names in loader:
        new=batch_diagnostic(model,tokenizer,policy,val_bindings,frequency,batch,names)
        for row in new:
    ...
    metadata.update(train_text_identity_verified_samples=train_identity_count,
        validation_text_identity_verified_samples=val_identity_count,policy_identity='mask_filename',metric_identity='image_filename')
