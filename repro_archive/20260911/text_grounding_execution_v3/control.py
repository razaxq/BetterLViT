...
REMOTE='/root/text_grounding_a_v3_20260911'
PYTHON='/root/autodl-tmp/envs/betterlvit-paper/bin/python'
KEY='C:/Users/dtftn/.ssh/seetacloud_betterlvit_ed25519'
FILES=('run_diagnostic.py','analysis.py','text_policy.py','routing.py','data_identity.py','check_policy.py',
...
def verify():
    ...
    assert record['train_text_identity_verified_samples']==5716 and record['validation_text_identity_verified_samples']==1429
    assert record['policy_identity']=='mask_filename' and record['metric_identity']=='image_filename'
    reference={r['name']:r for r in json.loads((HERE/'r2_validation.json').read_text())['records']}
    assert set(reference)=={r['name'] for r in rows}
    for row in rows:
        assert row['mask_name'].replace('mask_','')==row['name']
        for metric in ('iou','dice','precision','recall'):
            assert abs(row['baseline'][metric]-reference[row['name']][metric])<1e-12
