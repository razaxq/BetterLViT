...
def main():
    ...
    bindings=build_bindings(names,texts)
    images=sorted(p.name for p in (a.train_folder/'img').iterdir() if p.is_file())
    assert sorted(bindings)==images and len(bindings)==len(names)
    for image,case in bindings.items():
        assert case['mask_name'].replace('mask_','')==image and case['text']==texts[case['mask_name']]
    ...
    result=dict(kind='train_only_intervention_policy_check',passed=True,samples=len(names),
        ...
        training_image_text_bindings_verified=len(bindings),
        masked_workbook_keys=sum(n!=v['mask_name'] for n,v in bindings.items()),
        policy_identity='mask_filename',metric_identity='image_filename',
        note='Strict complete-sentence parser; abstentions may exceed the earlier location-only corpus audit.')
    assert result['policy_sha256']==manifest['registered_text_policy_sha256']
    assert result['generated_variants_sha256']==manifest['expected_generated_train_variants_sha256']
