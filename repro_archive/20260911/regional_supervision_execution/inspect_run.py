...
def main():
    p=argparse.ArgumentParser();p.add_argument('--label',choices=('rs1','rs2','rs3'),required=True)
...
''')
    ...
    if phase=='complete':
        ...
        if usable:
            ...
            last=value['timing'][-1]
            end=last['ended_unix']+(80-last['epoch'])*rate+120
            value['forecast']=dict(mean_epoch_seconds=rate,predicted_training_end_unix=end,
                final_check_unix=math.ceil((end+720)/60)*60,remaining_audit_allowance_seconds=120)
