...
posthoc=json.loads((HERE/'posthoc_collapse.json').read_text());assert posthoc['checkpoint_load_and_hash_verified']
...
assert sha=='49905dbbdc644454a37a4a49098db0d5df5fe75a' and proof['source_git_commit']==sha
for name,meta in runtime['artifacts'].items():assert digest(HERE/'results'/name)==meta['sha256']
assert subprocess.check_output(['git','rev-parse','pilot-r2-text-residual-b-v1-20260911'],cwd=DOCS,text=True).strip()==sha
stage=Path('D:/BetterLViT/.codex_tmp/hf_text_head_'+prefix);stage.mkdir(exist_ok=True)
...
mapping += [(HERE/name,prefix+'/'+name) for name in ('manifest.json','PROTOCOL.md','split.json','REPORT.md','posthoc_collapse.json','diagnose_collapse.py')]
...
manifest=dict(source_git_commit=sha,bucket_prefix=prefix,classification='completed_train_internal_frozen_head_pilot',
    formal_test_result=False,official_validation_accessed=False,test_split_accessed=False,files=files,
    checkpoint_load_verified=True,steps_per_head=1024,additive_only=True)
