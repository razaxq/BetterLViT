...
## 当前：v2在首个Train批次失败，已封存

22:17:42.221悉尼时间首次预约检查确认运行已于22:10:31.829停止，原因是诊断代码以DataLoader返回的图像名索引使用mask文件名作键的文字表。尚未进入Val，未更新模型、未访问Test；观察距失败430.393秒（7.17分钟），本次只用1次检查，余下检查不再使用。完整异常在`inspection_1.json`、`failed_runtime.json`、`failed_run.log`。这是诊断接口错误，不是文字方法效果的负结果。

修复在独立目录[文字诊断v3](../text_grounding_execution_v3/README.md)进行。旧源、服务器目录和所有回执保留，不修改v2或放宽R2逐图复现容差。
