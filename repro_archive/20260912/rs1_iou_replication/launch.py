...
def main():
    p=argparse.ArgumentParser();p.add_argument('--label',choices=('rs1s2027','rs1s3407'),required=True);label=p.parse_args().label
    ...
    if source['previous_label']:
        previous_label=source['previous_label']
        previous_path=HERE/(previous_label+'_final_snapshot.json')
        if not previous_path.exists():previous_path=HERE/(previous_label+'_first_snapshot.json')
        previous=json.loads(previous_path.read_text())
        assert previous['runtime']['phase']=='complete'
        assert (HERE/(previous_label+'_results/README.md')).exists(),'Archive and verify the previous run before dispatching the next one'
        audit=json.loads((HERE/(previous_label+'_results/independent_verification.json')).read_text())
        backup=json.loads((HERE/(previous_label+'_results/hf_upload_verified.json')).read_text())
...
if receipt.exists():
    ...
    with (run.parent/(run.name+'_launcher.log')).open('x') as log:
        child=subprocess.Popen(['/root/autodl-tmp/envs/betterlvit-paper/bin/python','-u','tools/run_regional_experiment.py','--run',str(run)],
