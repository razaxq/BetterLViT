...
三个匹配种子的六组训练全部完成。R2平均Val IoU提高0.5440个百分点，三个种子均正向，全部注册复验条件通过。最后训练末检已完成，以下首次检查等条目均为历史记录。完整结果见 [三种子汇总](THREE_SEED_SUMMARY.md)，接续已授权的 [固定协议Test](../../20260910/recipe_test/README.md)。

## R2-3407首次检查

2026-09-09 23:03:41.164（悉尼）的第1次短连接确认最后一组R2-3407训练健康：完成4/80轮、正在第5轮，SHA/manifest一致、跟踪文件干净，未访问Test。排除第1轮后，第2–4轮平均214.507214秒，预计2026-09-10 03:33:39.592结束训练；同一heartbeat `betterlvit-r2` 已改约9月10日03:46进行第2次也是最后一次检查。预算1/2，之后不再中途查询或保持连接。预测不保证实际间隔，末检须按真实结束时间核对≤30分钟。首检快照、预测、state和定时记录均已归档；当前没有R2-3407最终结果，须完整结束后再汇总三个配对。

## R2-3407已提交

C4-3407完整结果归档并推送后，最后一组R2-3407于2026-09-09 22:47:10.844（悉尼）提交，PID171080，来源 `7b498ba94fed230731d1daf9a5f4de042e4e71f2`，tag `experiment-r2-recipe-80e-seed3407-20260909`。当前只有启动回执，尚未检查健康，独立预算0/2。同一heartbeat `betterlvit-r2` 已改约今天23:03首次检查，再按实测完整轮时预测末检。启动前系统盘可用5,737,332,736字节，全部原模型保留。保持原注册配方；本组完成后生成3407配对及三种子汇总，再按既定门槛决定固定协议Test。

## C4-3407完成

已完成80轮，Best79，Val macro IoU **0.7203136633**、Dice **0.8188524577**，1429样本、固定阈值0.5，未访问Test。训练22:32:01（悉尼）结束，Val导出22:33:51完成，末检22:43:51，结束至末检710.132秒；预算2/2，符合30分钟约定。完整日志、实际80轮LR、逐图记录及Best/Last来源已核对，见[c4s3407_results](c4s3407_results/README.md)。三个种子的C4均已完成，接续最后一组R2-3407；尚不能汇总第三个配对增益。

## C4-3407首次检查

2026-09-09 18:00:35.268（悉尼）的第1次短连接确认C4-3407训练健康：完成4/80轮、正在第5轮，SHA/manifest一致、跟踪文件干净，未访问Test。排除第1轮后，第2–4轮平均214.745656秒，预计今天22:30:55.545结束训练；同一heartbeat `betterlvit-r2` 已改约22:43进行第2次也是最后一次检查。预算1/2，之后不再中途查询或保持连接。预测不保证实际间隔，末检须按真实结束时间核对≤30分钟。首检快照、预测、state和定时记录均已归档；当前没有3407最终结果。
