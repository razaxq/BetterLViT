...
parser.add_argument('--label', choices=('c4s2027','r2s2027','c4s3407','r2s3407'), required=True)
label = parser.parse_args().label
root = Path('D:/BetterLViT/outputs/recipe_replication_20260909')
snapshot_path = root / f'{label}_first_snapshot.json'
...
forecast = {
    ...
    'assumptions': [
        'Future full epochs have approximately the mean duration of observed completed epochs excluding epoch 1.',
...
(root / f'{label}_forecast.json').write_text(json.dumps(forecast, indent=2) + '\n', encoding='utf-8')
