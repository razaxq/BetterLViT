"""Additively preserve completed decoder controls; verify both Bucket listings."""
import argparse
import base64
import hashlib
import json
...
def main():
    p=argparse.ArgumentParser();p.add_argument('--label',choices=('m1','m2'),required=True);label=p.parse_args().label
    ...
    supplements={}
    for name in ('REPORT.md','independent_verification.json','paired_comparison.json'):
        supplements['analysis/'+name]=base64.b64encode((folder/name).read_bytes()).decode()
    for name in ('PROTOCOL.md','sources.json','analyze.py','preflight_verified.json','github_sources_verified.json'):
        supplements['analysis/'+name]=base64.b64encode((HERE/name).read_bytes()).decode()
    if label=='m2':
        supplements['analysis/m2_vs_m1.json']=base64.b64encode((folder/'m2_vs_m1.json').read_bytes()).decode()
...
if manifest_path.exists():
    ...
    mapping.append((repo/'experiment_manifests/active_decoder.json',prefix+'/manifest.json'))
    ...
    manifest=dict(source_git_commit=sha,bucket_prefix=prefix,classification='completed_validation_only_80e_decoder_control',
...
''',timeout=180)
    ...
    stage=str(Path(source['remote_run']).parent).replace('\\','/')+'/hf_staging/'+prefix
