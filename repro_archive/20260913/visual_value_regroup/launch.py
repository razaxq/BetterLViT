...
if label=='m2':
    for name in ('independent_verification.json','hf_upload_verified.json','download_xet_verified.json','github_archive_verified.json'):
        assert read(HERE/'m1_results'/name)['verified']
...
if label=='m2':
    ...
    history=read(HERE/'m1_results/runtime.json')
    assert history['phase']=='complete' and history['training_rc']==history['validation_rc']==0
    assert history['source_git_commit']==read(HERE/'sources.json')['m1']['source_git_commit']
    ratio=proof['candidates']['m2']['steady_seconds_per_batch']/proof['candidates']['m1']['steady_seconds_per_batch']
    reference='completed_m1_actual_runtime_scaled_by_m2_over_m1_preflight'
