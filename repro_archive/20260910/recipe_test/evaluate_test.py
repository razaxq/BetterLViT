...
def main():
    ...
    gate_path = Path(os.environ["RECIPE_TEST_GATE"])
    gate = json.loads(gate_path.read_text())
    if not gate["passed"] or gate["predeclared_seeds"] != [1219, 2027, 3407]:
        raise RuntimeError("Three-seed validation gate has not passed")
    if __import__("hashlib").sha256(gate_path.read_bytes()).hexdigest() != os.environ["RECIPE_TEST_GATE_SHA256"]:
        raise RuntimeError("Gate content hash mismatch")
    ...
    if any(bool(getattr(config, k, False)) for k in ("visual_prior_enabled", "race_enabled", "race_pe_enabled", "bcdh_enabled", "cdrr_enabled", "text_use_lora")):
    ...
    result = {
        ...
        "gate_sha256": os.environ["RECIPE_TEST_GATE_SHA256"],
