...
模型保留。提交时系统盘可用4,024,750,080字节，共享fs实际18,559,782,256字节，硬上限20,000,000,000；Test不复制权重、无外部下载。HF凭据失效是已告知旧问题，未有认证变化时不重试认证/上传、不重复通知。普通pending/healthy无有意义变化保持安静，只在完成、失败、需要用户处理或实际重大变化时通知。
