"""Immutable deployment, short GPU preflight, disconnected launch and bounded collection."""
import argparse
import base64
...
REMOTE='/root/local_reencode_20260910'
PYTHON='/root/autodl-tmp/envs/betterlvit-paper/bin/python'
KEY='C:/Users/dtftn/.ssh/seetacloud_betterlvit_ed25519'
FILES=('run.py','model.py','analysis.py','check_analysis.py','manifest.json','PLAN.md','r2_validation.json')


def remote(code):
    r=subprocess.run(['ssh','-i',KEY,'-p','21465','-o','BatchMode=yes','-o','ConnectTimeout=15',
        'root@connect.westb.seetacloud.com',PYTHON+' -'],input=code,text=True,encoding='utf-8',capture_output=True,timeout=85)
...
def source():
    ...
    for name in FILES:
        ...
        assert blob==(HERE/name).read_bytes(),name
...
with (root/'preflight.log').open('w') as log:
    job=subprocess.run(['/root/autodl-tmp/envs/betterlvit-paper/bin/python','-u',str(root/'run.py'),
        '--preflight','--output',str(root/'preflight.json'),'--source-sha',SHA],cwd=root,env=env,
        stdin=subprocess.DEVNULL,stdout=log,stderr=subprocess.STDOUT,timeout=65)
if job.returncode:raise RuntimeError((root/'preflight.log').read_text()[-7000:])
r=json.loads((root/'preflight.json').read_text());r.update(shared_fs_bytes=shared)
(root/'preflight_receipt.json').write_text(json.dumps(r))
print(json.dumps(r))
''')
    assert result['status']=='ok' and result['source_git_commit']==sha
    for name in ('run.py','model.py','analysis.py','check_analysis.py'):assert result['scripts_sha256'][name]==digest(HERE/name)
    write_json(OUT/'preflight.json',result)
    print(json.dumps({k:v for k,v in result.items() if k!='fit'}))


def launch():
    sha,_=source();pf=json.loads((OUT/'preflight.json').read_text())
    assert pf['source_git_commit']==sha
    assert not (OUT/'launch_attempt.json').exists(),'Reconcile receipt before retry'
...
pf=json.loads((root/'preflight_receipt.json').read_text());assert pf['source_git_commit']==SHA and pf['status']=='ok'
assert shutil.disk_usage('/root').free>1_000_000_000
assert not subprocess.check_output(['nvidia-smi','--query-compute-apps=pid','--format=csv,noheader,nounits'],text=True).strip()
env=dict(os.environ,CUBLAS_WORKSPACE_CONFIG=':4096:8',PYTHONHASHSEED='1219')
started=time.time()
with (root/'run.log').open('x') as log:
    p=subprocess.Popen(['/root/autodl-tmp/envs/betterlvit-paper/bin/python','-u',str(root/'run.py'),
        '--output',str(root/'results'),'--source-sha',SHA],cwd=root,env=env,stdin=subprocess.DEVNULL,
        stdout=log,stderr=subprocess.STDOUT,start_new_session=True,close_fds=True)
receipt=dict(pid=p.pid,submitted_unix=started,source_git_commit=SHA,deployed_sha256=HASHES,
    system_free_bytes=shutil.disk_usage('/root').free,remote_directory=str(root))
(root/'launch_receipt.json').write_text(json.dumps(receipt));print(json.dumps(receipt))
''')
    expected=result['submitted_unix']+pf['predicted_run_seconds'];check=math.ceil((expected+180)/60)*60
    state=dict(phase='submitted',source_git_commit=sha,inspections_completed=0,maximum_inspections=2,
...
def collect():
    ...
    assert time.time()>=state['planned_check_unix'],'Honor the predicted check time'
...
if r['phase']=='running':
    ...
    out['progress']=json.loads(progress.read_text()) if progress.exists() else {}
else:out['log_tail']=(root/'run.log').read_text(errors='replace')[-8000:]
...
''')
    ...
    if state['phase']=='complete':
        ...
        for name,b64 in files.items():
            assert Path(name).name==name
            (target/name).write_bytes(base64.b64decode(b64))
            assert digest(target/name)==result['runtime']['artifacts'][name]['sha256']
        write_json(target/'runtime.json',result['runtime'])
        (target/'run.log').write_text(result['log_tail'],encoding='utf-8')
        records=json.loads((target/'records.json').read_text());summary=json.loads((target/'summary.json').read_text())
        assert records['source_git_commit']==state['source_git_commit'] and not records['test_split_accessed']
        assert records['baseline_state_unchanged'] and len(records['records'])==1429
        assert records['baseline_per_image_max_difference']<1e-12 and summarize(records['records'])==summary
        assert all(r['selected_pixels']==10240 for r in records['records'])
        state.update(seconds_after_completion=result['observed_unix']-result['runtime']['completed_unix'],proceed=summary['proceed'])
        state['completion_check_within_30min']=0<=state['seconds_after_completion']<=1800
        write_json(target/'independent_verification.json',dict(verified=True,source_git_commit=state['source_git_commit'],
                   checks=['artifact_sha256','exact_cpu_summary','1429_baseline_metrics','mask_budget','frozen_state','no_test']))
    elif state['phase']=='running' and state['inspections_completed']<2:
        remaining=result.get('progress',{}).get('remaining_training_seconds')
        assert remaining is not None,'Cannot safely predict final check without timing'
        pf=json.loads((OUT/'preflight.json').read_text())
        state['planned_check_unix']=math.ceil((time.time()+remaining+pf['evaluation_seconds_per_image']*1685+300)/60)*60
        state['planned_check_sydney']=datetime.fromtimestamp(state['planned_check_unix'],ZoneInfo('Australia/Sydney')).isoformat()
    write_json(OUT/'state.json',state)
    print(json.dumps(dict(state=state,runtime=result['runtime'],files_downloaded=len(files))))


if __name__=='__main__':
    parser=argparse.ArgumentParser();parser.add_argument('action',choices=('preflight','launch','collect'))
