...
for role,path in (('best',best),('last',last)):
    ...
    assert ck['visual_aux_mode']==runtime['manifest']['visual_aux_mode']
    row=dict(role=role,path=str(path),bytes=path.stat().st_size,source_git_commit=ck['source_git_commit'],
        epoch=ck['epoch'],best_epoch=ck['best_epoch'],visual_aux_mode=ck['visual_aux_mode'],training_recipe=ck['training_recipe'])
    models.append(row)
    if role=='last':
...
''',timeout=180)
    ...
    script=Path(source['local_repository'])/'tools/compare_visual_aux.py'
    ...
    if label=='s2':
        subprocess.run(['python','-X','utf8',str(script),'--control',str(HERE/'s1_results/validation.json'),
            '--candidate',str(destination/'validation.json'),'--output',str(destination/'s1_vs_s2.json'),'--regional-increment'],check=True)
