...
experiment="${1:-c1_bcdh_control}"
seed="${2:-1219}"
epochs="${3:-40}"
batch_size="${4:-16}"
case "$experiment" in
  c1_bcdh_control|p6_bcdh_r_v1) ;;
  *) echo "Unsupported validation pilot: $experiment" >&2; exit 2 ;;
esac
...
lock_file="/root/autodl-tmp/betterlvit_validation_training.lock"
...
nohup "$runner" "$repo_root" "$lock_file" "$python_bin" \
  "$status_file" "$experiment" >"$stdout" 2>"$stderr" </dev/null &
launcher_pid=$!
sleep 3
