...
**执行更新：** 两组20轮特征探针已完成。CXformer-S的Val macro IoU/Dice为0.530981/0.667679，同构DINOv2-S为0.475959/0.616265；这是特征读出诊断，不是BetterLViT增益。两次检查已完成，末次分别距结束约11.0/8.3分钟。P12正式版本和C9控制的重复GPU预检均通过；P12的80轮实验已于悉尼14:21提交后台，源码`5d09913d46863073cba93159af4ed61f88fdd96e`，14:37:48首次检查确认第4轮完成、第5轮训练中，平均轮时224.28秒；预测19:20:55训练结束，唯一末次检查已改为19:33（检查预算1/2）。C9源码已冻结但待P12通过C4门槛才训练。没有新增Test成绩。完整版本、逐图结果与检查安排见[执行记录](../repro_archive/20260908/visual_prior_execution/执行记录.md)。
...
研究报告完成时仅进行了公开元数据审计；后续诊断与训练推进以上方执行更新为准。当前仍无本方向完整BetterLViT的IoU增益证据。
