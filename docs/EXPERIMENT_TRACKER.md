...
修复在独立[text_grounding_execution_v3](../repro_archive/20260911/text_grounding_execution_v3/README.md)：显式mask→image映射及原文字token一致性检查，并修复Val参照名称集合对应。11项本地与服务器CPU测试、5716个Train样本映射均通过，生成文字SHA与原静态审计完全一致。保留权重、15条件、阈值及复现容差。

**v3已于22:25:53.479后台重启：** PID273120，来源`da17b3fbef0ca4ac6343575cf2af18fab3c76efb`，标签`diagnostic-r2-text-path-a-v3-20260911`，源码及标签已推送GitHub。11份部署文件哈希一致、启动前GPU空闲；目前只有提交回执，未确认GPU预检或得到新IoU/Test成绩。同一heartbeat改约**22:31首次检查**并核验，检查0/2。[v3回执](../repro_archive/20260911/text_grounding_execution_v3/launch.json)。以下v2提交记录作为历史。
...
## RS1/RS2/RS3首轮完成并备份（2026-09-11）

**RS3完成并核验：** 21:48:32.654完成80轮，21:50:19.998完成Val，Best67；22:01:44.563末检距训练结束13.20分钟，检查2/2、训练/Val退出码0。来源`f79842331e4e5e41526ed66da31ec174ea4a61b2`、学习率、Best、逐图指标及Train诊断状态恢复均核验，未访问Test。

RS3 Val IoU **72.8557%**、Dice **82.5468%**；相对R2 IoU **+0.1903 pp**、95%区间[−0.0668,+0.4506]，综合门4/7通过、整体失败。相对RS1 IoU−0.2207 pp，区域增量门失败；RS3−RS2仅+0.0162 pp、区间[−0.2626,+0.3069]，无分组独立增益证据。三组汇总见[FIRST_ROUND_REPORT.md](../repro_archive/20260911/regional_supervision_execution/FIRST_ROUND_REPORT.md)；保留RS1正向单种子Val IoU证据，不称稳定Test成果。

RS3已在HF Bucket `f7984233/`核验21文件、1,694,438,969字节，11项下载run产物匹配；Best/Last保留，fs18,559,782,256字节、训练盘余2,567,913,472字节。按用户新授权接续文字A诊断，不扩展RS4或150轮。以下首检/提交记录保留为历史，不作为最新状态。
